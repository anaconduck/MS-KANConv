...
==========================================
Visualize learned KAN B-spline activations and channel attention.
"""
...
def plot_kan_activations(model, save_path, dataset_name=""):
    """
    Visualize the learned B-spline activation functions.
    Shows how different channels learn different nonlinearities.
    """
    ...
    # Plot a selection of activations
    ...
    # Standard activation functions for comparison
    ...
    for name, kan_act in list(activations.items())[:8]:
        ...
        for ch in range(num_ch_to_show):
            ax.plot(x_vals.numpy(), y_vals[ch].numpy(),
                    color=colors[ch], alpha=0.7, linewidth=1.5,
                    label=f"ch{ch}")

        # Reference lines
        ax.plot(x_ref.numpy(), relu_ref, 'k--', alpha=0.3,
                linewidth=1, label="ReLU")
        ax.plot(x_ref.numpy(), silu_ref, 'r--', alpha=0.3,
                linewidth=1, label="SiLU")
...
def plot_channel_attention(model, test_dataset, device, save_path,
                           dataset_name="", activity_labels=None):
    """
    Visualize SE attention weights across different activity classes.
    """
    model.eval()
    model.to(device)
    ...
    def make_hook(block_name):
        def hook_fn(module, input, output):
            # output is the reweighted tensor; extract weights
            x = input[0]  # original input
            w = x.mean(dim=-1)
    ...
    for block_name in attention_weights:
        ...
        sns.heatmap(
            class_weights, annot=False, cmap="YlOrRd",
            xticklabels=ch_labels, yticklabels=class_names,
            ax=ax, vmin=0, vmax=1,
        )
        ax.set_title(
...
def run_interpretability():
    ...
    for dataset_name in ["uci_har", "mhealth"]:
        ...
        # 1. Plot KAN activations
        act_path = os.path.join(
            FIGURES_DIR, f"kan_activations_{dataset_name}.png"
        )
        plot_kan_activations(model, act_path, dataset_name)

        # 2. Plot channel attention
        attn_path = os.path.join(
            FIGURES_DIR, f"channel_attention_{dataset_name}.png"
        )
        plot_channel_attention(
            model, test_ds, device, attn_path,
            dataset_name=dataset_name,
